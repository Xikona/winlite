@echo off & color 88 & mode con cols=45 lines=15 & title AIO
echo. ================
echo. [1] nvidia	=
echo. [2] script	=
echo. [3] icons	=
...
:: extras
( label C:Disk & md "%windir%\Extras"
copy "Files\bin\service.bat" "%windir%\Extras" ) >nul 2>&1
:: theme
( del "%windir%\Resources\Themes\aero.theme" /s /q
...
copy "Files\bin\user-192.png" "%programdata%\Microsoft\User Account Pictures"
...
copy "%windir%\Resources\Themes\Gray\Shell\NormalColor\shellstyle.dll" "%windir%\Resources\Themes\aero\Shell\NormalColor\shellstyle.dll"
:: console
...
:: taskmanager
md "%localappdata%\Microsoft\Windows\TaskManager"
copy "Files\bin\settings.json" "%localappdata%\Microsoft\Windows\TaskManager\settings.json"
:: explorer
...
copy "Files\bin\quick_access" "%appdata%\Microsoft\Windows\Recent\AutomaticDestinations\f01b4d95cf55d32a.automaticDestinations-ms"
:: folder_view
...
reg add "HKCU\Software\StartIsBack" /t reg_dword /v "WelcomeShown" /d "3" /f
...
reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" /t reg_dword /v "Start_SearchFiles" /d "0" /f
reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer" /t reg_dword /v "NoTrayContextMenu" /d "0" /f
:: cuteborders
...
:: context_menu
rd "%localappdata%\Microsoft\Windows\WinX" /s /q & md "%localappdata%\Microsoft\Windows\WinX"
...
:: hotkeys
reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" /t reg_expand_sz /v "DisabledHotkeys" /d "QSXD" /f
reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Policies\System" /t reg_dword /v "DisableLockWorkstation" /d "1" /f
...
copy "Files\bin\ahk.ahk" "%windir%\Extras\Shell"
reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Run" /t reg_sz /v "ahk" /d "%windir%\Extras\Shell\ahk.ahk" /f
:: sab 3.7.11
Files\sab.exe
...
:chocoinst
powershell -ExecutionPolicy ByPass -command Set-ExecutionPolicy Bypass -Scope Process -Force; [System.Net.ServicePointManager]::SecurityProtocol = [System.Net.ServicePointManager]::^
SecurityProtocol -bor 3072; iex ((New-Object System.Net.WebClient).DownloadString('https://community.chocolatey.org/install.ps1')) >nul 2>&1 & exit /b
:chocoset
:: puntoswitcher
( md "%appdata%\Yandex\Punto Switcher\User Data"
copy "Files\bin\preferences.xml" "%appdata%\Yandex\Punto Switcher\User Data\preferences.xml" ) >nul 2>&1
::choco install startallback --version=3.7.11 -y
choco install puntoswitcher -y
choco install firefox --params "/NoAutoUpdate /NoMaintenanceService" -y
choco install qbittorrent -y
choco install onlyoffice -y
choco install autohotkey.install -y
choco install choco-cleaner -y
::choco uninstall autohotkey.portable --force
cls & choco list & echo. & echo.Press any key... & pause >nul & cls
( schtasks /change /disable /tn "\choco-cleaner"
:: onlyoffice
...
:: autohotkey
reg add "HKCR\AutoHotkeyScript" /t reg_expand_sz /v "FriendlyTypeName" /d "Script" /f
:: firefox
rd "%appdata%\Mozilla\Firefox\Profiles" /s /q & md "%appdata%\Mozilla\Firefox\Profiles"
"%programfiles%\Mozilla Firefox\firefox.exe" --createprofile user ) >nul 2>&1
"%programfiles%\Mozilla Firefox\firefox.exe" -P & echo. & echo.Press any key... & pause >nul & taskkill /f /im firefox.exe >nul 2>&1
set folder=%appdata%\Mozilla\Firefox\Profiles
for /f %%i in ( ' dir /a:d/b "%folder%" ' ) do if exist "%folder%\%%i\." xcopy /e /y /q /i "Files\firefox\chrome" "%folder%\%%i\chrome" >nul
for /f %%i in ( ' dir /a:d/b "%folder%" ' ) do if exist "%folder%\%%i\." copy "Files\firefox\user.js" "%folder%\%%i\user.js" >nul
:: taskbar_start
...
attrib +h /s /d "%programdata%\Microsoft\Windows\Start Menu\Programs\*.*" & attrib +h "C:\tools"
attrib +h /s /d "%appdata%\Microsoft\Windows\Start Menu\Programs\*.*"
nircmd shortcut "%appdata%\Microsoft\Windows\Network Shortcuts" "%programdata%\Microsoft\Windows\Start Menu\Programs\System" "Shortcuts"
nircmd shortcut "%windir%\Extras\WinRAR\WinRAR.exe" "%programdata%\Microsoft\Windows\Start Menu\Programs\System" "WinRAR" "" "%windir%\SystemResources\zipfldr.dll.mun"
nircmd shortcut "%windir%\Extras\Color\instanteyedropper.exe" "%programdata%\Microsoft\Windows\Start Menu\Programs\System" "Color"
nircmd shortcut "%programdata%\Microsoft\Windows\Start Menu\Programs\qBittorrent\qBittorrent.lnk" "%programdata%\Microsoft\Windows\Start Menu\Programs" "qBittorrent"
nircmd shortcut "%programdata%\Microsoft\Windows\Start Menu\Programs\ONLYOFFICE\ONLYOFFICE.lnk" "%programdata%\Microsoft\Windows\Start Menu\Programs" "Office"
nircmd shortcut "%programdata%\chocolatey\bin\choco-cleaner.bat" "%programdata%\Microsoft\Windows\Start Menu\Programs\System" ^
"Cleaner" "" "%allusersprofile%\chocolatey\lib\choco-cleaner\tools\choco-cleaner.ico"
attrib -h "%programdata%\Microsoft\Windows\Start Menu\Programs\System"
attrib -h "%programdata%\Microsoft\Windows\Start Menu\Programs\System\*.*"
attrib -h "%programdata%\Microsoft\Windows\Start Menu\Programs\qBittorrent.lnk"
attrib -h "%programdata%\Microsoft\Windows\Start Menu\Programs\Office.lnk"
attrib -h "%appdata%\Microsoft\Windows\Start Menu\Programs\Startup\Punto Switcher.lnk"
taskkill /f /im explorer.exe & start explorer.exe ) >nul 2>&1 & exit /b
